...
$fechahoy=date("Y-m-d");
$sel_fact="INSERT INTO eefacturastmp (bbcodfactura,fecha) VALUE ('','$fechahoy')";
...
if ($setnumbol==1)
{
	$sel_articulos="UPDATE eefacturastmp SET bbcodfactura='$numeracionboleta' WHERE bbcodfactura='$bbcodfacturatmp'";
	$rs_articulos=mysql_query($sel_articulos);
	$bbcodfacturatmp=$numeracionboleta;
}
?>
<html>
	<head>
	...
	<body>
		<div id="pagina">
			<div id="zonaContenido">
				<div align="center">
				<div id="tituloForm_nf" class="header">INSERTAR BOLETA </div>
				...
				<form id="formulario" name="formulario" method="post" action="guardar_factura.php">
					<table class="fuente8" width="98%" cellspacing=0 cellpadding=3 border=0>
						...
						<tr>
				            ...
				            <td width="3%">NIF / RUC</td>
				            <td width="64%"><input NAME="nif" type="text" class="cajaMedia" id="nif" size="20" maxlength="15" readonly></td>
							<?php
					  	$query_vendedores="SELECT * FROM vendedores ORDER BY nombrevendedor ASC";
						$res_vendedores=mysql_query($query_vendedores);
						$bbcontador=0;
					        ?>
						<tr>
							<td width="15%">Vendedor</td>
							<td width="43%"><select id="cboVendedores" name="cboVendedores" class="comboGrande">
							<option value="0">Seleccione un Vendedor</option>
								<?php
								while ($bbcontador < mysql_num_rows($res_vendedores)) { ?>
								<option value="<?php echo mysql_result($res_vendedores,$bbcontador,"codvendedor")?>"><?php echo mysql_result($res_vendedores,$bbcontador,"nombrevendedor")?></option>
								<? $bbcontador++;
								} ?>				
								</select>							</td>
				        </tr>
...
<!--
							...
							<td width="3%">Vendedor</td>
				            <td width="10%"><input NAME="cvendedor" type="text" class="cajaMedia" id="cvendedor" size="20" maxlength="15" readonly></td>
						</tr>	
						-->	
        ...
        <script type="text/javascript">
					...
					button     : "Image1"
				            ...
				            <td width="3%">IGV</td>
				            <td width="64%"><input NAME="iva" type="text" class="cajaPequena" id="iva" size="5" maxlength="5" value="<? echo $ivaimp?>" onChange="cambio_iva()"> %</td>
						</tr>
					</table>										
			  </div>
